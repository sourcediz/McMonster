...
const PresetFont: React.FC<TpresetFontProps> = ({text, fontStyle, remSize}) => {
  const rem = useRem();
  return (
    <Text style={[{color: '#fff'}, fontStyle, {fontSize: rem(remSize, true)}]}>
      {text}
    </Text>
  );
};

export const Header: React.FC<TpresetFontProps> = ({
  text,
  fontStyle,
  remSize,
}) => {
...
export const H1: React.FC<fontProps> = ({text, fontStyle}) => {
  return <Header text={text} fontStyle={fontStyle} remSize={2.5} />;
};

export const H2: React.FC<fontProps> = ({text, fontStyle}) => {
  return <Header text={text} fontStyle={fontStyle} remSize={2} />;
};

export const H3: React.FC<fontProps> = ({text, fontStyle}) => {
  return <Header text={text} fontStyle={fontStyle} remSize={1.75} />;
};

export const H4: React.FC<fontProps> = ({text, fontStyle}) => {
  return <Header text={text} fontStyle={fontStyle} remSize={1.5} />;
};

export const H5: React.FC<fontProps> = ({text, fontStyle}) => {
  return <Header text={text} fontStyle={fontStyle} remSize={1.25} />;
};

export const H6: React.FC<fontProps> = ({text, fontStyle}) => {
  return <Header text={text} fontStyle={fontStyle} remSize={1} />;
};
