...
const MapScreen = () => {
  ...
  const [mapLocation, setMapLocation] = React.useState<Tlocation>({
    lat: 0,
    lng: 0,
  });
  const [searchLocation, setSearchLocation] = React.useState<Tlocation>({
    lat: 0,
    lng: 0,
  });
  ...
  const [selectedMonster, setSelectedMonster] = React.useState<Tmonster | null>(
    null,
  );
  ...
  React.useEffect(() => {
    if (userLocation.lng != 0) {
      getMacdonals(userLocation).then(r => {
  ...
  React.useEffect(() => {
    ...
    if (searchLocation.lng != 0) {
      getMacdonals(searchLocation).then(r => {
        const monsters = r.map(place => {
          const monster = generateMonster(place.place_id, place.rating);
          monster.location = place.geometry.location;
          monster.address = place.vicinity;
